...
class GNNLayer(torch.nn.Module):
    def __init__(self, in_dim, out_dim, attn_dim, n_ent, n_rel, act=lambda x: x):
        ...
        self.Wq_embed = nn.Embedding(2 * n_ent, in_dim)
        self.W_attn = nn.Linear(attn_dim, 1, bias=False)
        self.W_h = nn.Linear(in_dim, out_dim, bias=False)

        # Disentanglement transformations - Residual connections
        self.sub_transform = nn.Sequential(
    ...
    def forward(self, q_sub, q_rel, hidden, edges, n_node, old_nodes_new_idx):
        ...
        h_qr = self.rel_embed(q_rel)[r_idx]
        h_q = self.Wq_embed(q_sub)[r_idx]

        # Disentangled message construction
        h_sub = self.sub_transform(hs)
        h_rel = self.rel_transform(hr)

        mess = h_q + h_sub + h_rel
        alpha_2 = torch.sigmoid(self.W_attn(nn.ReLU()(
            self.Ws_attn(h_sub) + self.Wr_attn(h_rel) + self.Wqr_attn(h_qr)
        )))
...
class GNNModel(torch.nn.Module):
    def __init__(self, params, loader):
        ...
        for i in range(self.n_layer):
            self.layers.append(GNNLayer(self.hidden_dim, self.hidden_dim, self.attn_dim, self.n_ent, self.n_rel, act=act))
        ...
        self.alpha = nn.Parameter(torch.Tensor([0.5]))  # Initial value for alpha

    def calculate_node_degrees(self, edges):
        n_node = len(set(edges[:, 0].tolist() + edges[:, 4].tolist()))
        deg = torch.zeros(n_node).cuda()
        node_counts = torch.bincount(edges[:, 4].long(), minlength=n_node).cuda()
        node_counts += torch.bincount(edges[:, 0].long(), minlength=n_node).cuda()
        deg.copy_(node_counts)

        return deg

    def soft_to_hard(self, i, hidden, nodes, n_ent, batch_size, old_nodes_new_idx, edges):
        n_node = len(nodes)
        ...
        node_degrees = self.calculate_node_degrees(edges)

        diff_node_logits = self.Ws_layers[i](hidden[bool_diff_node_idx].detach()).squeeze(-1)
        normalized_degrees = 1.0 - torch.exp(-node_degrees[diff_nodes[:, 0]])
        # Adjust weight for node degrees (alpha controls influence)
        combined_scores = diff_node_logits + self.alpha * normalized_degrees

        soft_all = torch.ones((batch_size, n_ent)) * float('-inf')
        soft_all = soft_all.cuda()
        soft_all[diff_nodes[:, 0], diff_nodes[:, 1]] = combined_scores
    ...
    def forward(self, subs, rels, mode='transductive'):
        ...
        for i in range(self.n_layer):
            ...
            if i < self.n_layer - 1:
                if self.increase:
                    hidde, bool_same_nodes = self.soft_to_hard(i, hidden, nodes, n_ent, n, old_nodes_new_idx, edges)
                else:
                    exit()
